...
weight = (np.log10(timeIndex + 1) * timeIndex * timeIndex - timeIndex)

highSlopeCum = highSlope * timeIndex
highLogDev = np.where(timeIndex > 2, np.log(weight) + highIntercept + highSlopeCum, np.nan)
print(highSlopeCum)
print(highLogDev)

# BOTTOM OF CHANNEL INTERCEPT & SLOPE CALCS
lowSlopeCum = lowSlope * timeIndex
lowLogDev = np.where(timeIndex > 2, np.log(weight) + lowIntercept + lowSlopeCum, np.nan)
print(lowSlopeCum)
print(lowLogDev)
